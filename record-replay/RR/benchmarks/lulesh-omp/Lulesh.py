...
class LULESH(Bench.MakeBenchmark):
    def __init__(self, prof, **kwargs):
        kwargs.pop("executable", None)
        kwargs.pop('resultsdir', None)
        kwargs.pop('root', None)
        super().__init__(prof,  "lulesh", os.path.split(os.path.realpath(__file__))[0], **kwargs)
    ...
    def command(self):
        return  f"{self.executableDir}/lulesh -s 128 -r 11 -b 1 -c 1"
