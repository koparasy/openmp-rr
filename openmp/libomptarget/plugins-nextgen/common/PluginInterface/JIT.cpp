...
JITEngine::compile(const __tgt_device_image &Image,
  ...
  std::lock_guard<std::mutex> Lock(CPUImageMapMutex);

  // Check if we JITed this image for the given compute unit kind before.
  CPUImageInfo &CPUII = CPUImageMap[ComputeUnitKind];
  if (__tgt_device_image *JITedImage = CPUII.TgtImageMap.lookup(&Image))
    return JITedImage;
  ...
  if (!ReplacementModuleFileName.isPresent()) {
    auto ModOrErr = createModuleFromImage(Image, CPUII.Context);
    if (!ModOrErr)
    ...
    auto ModOrErr = createModuleFromMemoryBuffer(MBOrErr.get(), CPUII.Context);
    if (!ModOrErr)
      return ModOrErr.takeError();
    Mod = ModOrErr->release();
  }
  ...
  CPUII.JITImages.push_back(std::move(*ImageMBOrErr));
  __tgt_device_image *&JITedImage = CPUII.TgtImageMap[&Image];
  JITedImage = new __tgt_device_image();
  *JITedImage = Image;

  auto &ImageMB = CPUII.JITImages.back();
